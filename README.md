...
- An ESP8266 or ESP32 board — [`examples/d1-mini.yaml`](examples/d1-mini.yaml) targets a cheap Wemos D1 Mini (ESP8266). The `gree` component itself also runs fine on ESP32; swap the YAML's `esp8266:`/`board: d1_mini` block for the matching `esp32:` block and your board's own pin names for the `uart:` section.
...
**⚠️ Do not leave it running on this broker long-term — it is not safe for real use.** EMQX's own docs say it plainly: this broker is for learning/testing only. There is no authentication, every message on it is visible to anyone else using it, and there's no guarantee of uptime. Anyone who guesses or finds your exact `tp:` topic prefix can see your AC's state and send it commands — including turning it on/off — with nothing stopping them. Use it only to confirm everything works end to end, then move to a broker with real authentication: either run your own (e.g. the free Mosquitto broker add-on built into Home Assistant, see below) or use EMQX Cloud/another broker with a real username and password.
